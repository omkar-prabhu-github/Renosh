  ...
  Future<void> _checkDonationStatus() async {
    try {
      ...
      if (doc.exists) {
        setState(() {
          _isDonated =
              (doc.data()!['quantity_surplus'] as num?)?.toInt() == 0;
          if (!_isDonated) {
            _quantityController.text =
                ((doc.data() as Map<String, dynamic>?)?['quantity_surplus'] as num?)?.toInt().toString() ?? '0';
          }
        });
      }
    } catch (e) {
  ...
  Future<void> _donateItem() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        final currentSurplus = (data?['quantity_surplus'] as num?)?.toInt() ?? 0;
